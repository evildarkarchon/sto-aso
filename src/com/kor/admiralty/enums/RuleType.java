...
public enum RuleType {
    ...
    private static SpecialAbility constructRule(RuleType rule, Reward reward) {
        switch (rule) {
            case WhenAlone:
                return new WhenAlone(reward);
            case FromAnyShip:
            case PerAnyShip:
            case PerShip:
                return new PerAnyShip(reward);
            case PerAnyShipNotSmallCraft:
                return new PerAnyShip(reward, true);
            case PerEng:
                return new PerShipCategory(reward, Role.Eng);
            case PerTac:
                return new PerShipCategory(reward, Role.Tac);
            case PerSci:
                return new PerShipCategory(reward, Role.Sci);
            case PerEngTac:
                return new PerShipCategory(reward, Role.Tac, Role.Eng);
            case PerEngSci:
                return new PerShipCategory(reward, Role.Eng, Role.Sci);
            case PerTacSci:
                return new PerShipCategory(reward, Role.Tac, Role.Sci);
            case MultiplyCriticalRate:
                return new ProcCritRate(reward);
            case Ignore:
            case All:
                return new AlwaysApply(reward);
            default:
                return new NoOp(reward);
        }
    }
